...
use BVC::Openflow::Action::PushMplsHeader;
...
my $action = new BVC::Openflow::Action::PushMplsHeader(order => 0);
...
$action = new BVC::Openflow::Action::Output(order => 2, port => $output_port);
$instruction->apply_actions($action);
