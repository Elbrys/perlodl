...
use Brocade::BSC::Openflow::Action::PushMplsHeader;
...
my $action = new Brocade::BSC::Openflow::Action::PushMplsHeader(order => 0);
...
$action = new Brocade::BSC::Openflow::Action::Output(order => 2, port => $output_port);
$instruction->apply_actions($action);
